...
public class ReaderServices {
	...
	public List<BooksDetails> getSubscribedBooks(String email) {
		...
		List<BooksDetails> list=restTemplate.getForObject("http://localhost:2222/app/v1/digitalbooks/readers/books/"+user.getUserId(),List.class);
		return list;
	}
	...
	public BooksDetails getBookDetails(String emailId, Long subscriptionId) {
		...
		BooksDetails booksDetails=restTemplate.getForObject("http://localhost:2222/app/v1/digitalbooks/readers/books/"+user.getUserId()+"/"+subscriptionId,BooksDetails.class);
		return booksDetails;
	}
	...
	public String getBookContent(Long subscriptionId) {
		...
		//User user  =userRepo.findByEmail(emailId);
		String content=restTemplate.getForObject("http://localhost:2222/app/v1/digitalbooks/readers/books/read/"+subscriptionId,String.class);
		return content;
	}

	public ResponseParms cancelSubscription(Long subscriptionId) {
		log.info("*********in  cancelSubscription******");
		ResponseParms content=restTemplate.getForObject("http://localhost:2222/app/v1/digitalbooks/readers/books/cancelsubscription/"+subscriptionId,ResponseParms.class);
		return content;
	}
	...
	public ResponseParms subscribeBook(SubscribeBook book) {
		...
		ResponseParms params=restTemplate.postForObject("http://localhost:2222/app/v1/digitalbooks/subscribe", entity, ResponseParms.class);
		return params;
	}

	public List<BooksDetails> search(String category, String title, String author, String price, String publisher) {
		log.info("in  search");
		List<BooksDetails> booksDetails=restTemplate.getForObject("http://localhost:2222/app/v1/digitalbooks/search?category="+category+"&title="+title+"&author="+author+"&price="+price+"&publisher="+publisher,List.class);
		return booksDetails;
	}
